...
test('computeForceLayout: Node mit x/y nutzt diese Position statt Force-Output', () => {
  const g = makeRpgGraph(
    {
      r: { id: 'r', title: 'R', x: 500, y: 700 },
      a: { id: 'a', title: 'A' },
    },
    [{ from: 'r', to: 'a', relation: 'parent_of' }]
  );
  const { positions } = computeForceLayout(g);
  assert.equal(positions.r.x, 500);
  assert.equal(positions.r.y, 700);
  assert.ok(Number.isFinite(positions.a.x));
  assert.ok(Number.isFinite(positions.a.y));
});

test('computeForceLayout: Bounding-Box deckt negative Manual-Positions ab', () => {
  const g = makeRpgGraph(
    {
      r: { id: 'r', title: 'R', x: -200, y: -300 },
      a: { id: 'a', title: 'A' },
    },
    [{ from: 'r', to: 'a', relation: 'parent_of' }]
  );
  const { minX, minY } = computeForceLayout(g);
  assert.ok(minX <= -200, `minX (${minX}) muss negative Manual-Position abdecken`);
  assert.ok(minY <= -300, `minY (${minY}) muss negative Manual-Position abdecken`);
});

test('computeForceLayout: nur x ohne y wird ignoriert (defensiv)', () => {
  const g = makeRpgGraph(
    { r: { id: 'r', title: 'R', x: 100 } }, // y fehlt
    []
  );
  const { positions } = computeForceLayout(g);
  assert.notEqual(positions.r.x, 100);
});
