...
export function computeForceLayout(graph, opts = {}) {
  ...
  const packed = packComponents(componentLayouts, padding, interComponentGap);

  // ----- Manual-Position-Override (Drag-and-Drop, 2026-05-04) -----
  // Nodes mit gesetzten x/y (vom User per Drag verschoben) ueberschreiben
  // den Force-Layout-Output. Force-Sim hat sie zwar mitsimuliert, aber die
  // finale Position kommt vom User. Bounding-Box wird neu berechnet weil
  // Manual-Positions auch negativ sein koennen.
  const manuals = extractManualPositions(nodeMap);
  if (manuals.size > 0) {
    for (const [id, pos] of manuals) {
      packed.positions[id] = { x: pos.x, y: pos.y };
    }
    let mnX = Infinity, mnY = Infinity, mxX = -Infinity, mxY = -Infinity;
    for (const id of Object.keys(packed.positions)) {
    ...
    if (Number.isFinite(mnX)) {
      return {
        ...
        width: Math.ceil(mxX - mnX + padding * 2),
        height: Math.ceil(mxY - mnY + padding * 2),
        minX: Math.floor(mnX - padding),
        minY: Math.floor(mnY - padding),
      };
    }
  }
  return packed;
...
/**
 * Extrahiert manuelle Positions aus dem Graph: Nodes mit `x`/`y` als
 * finite Numbers werden in eine Map zurueckgegeben. Nodes ohne valide
 * Position werden NICHT aufgenommen — die kriegen Auto-Layout.
 *
 * @param {Map<string, import('./rpg-quests-data.js').RpgNode>} nodeMap
 * @returns {Map<string, { x: number; y: number }>}
 */
function extractManualPositions(nodeMap) {
  /** @type {Map<string, { x: number; y: number }>} */
  const out = new Map();
  for (const [id, n] of nodeMap) {
    if (!n) continue;
    const x = /** @type {any} */ (n).x;
    const y = /** @type {any} */ (n).y;
